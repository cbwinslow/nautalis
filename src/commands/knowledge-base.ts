...
export function registerKnowledgeBaseCommand(program: Command): void {
  ...
  kbCmd
    ...
    .action(
      async (
        ...
        try {
          await withSpan('nautalis.command.kb.create', { 
            title, 
  ...
  kbCmd
    ...
    .action(
      async (
        ...
        try {
          await withSpan('nautalis.command.kb.search', { 
            query, 
            category: options.category || '',
  ...
  kbCmd
    ...
    .action(
      async (options: {
        ...
        try {
          await withSpan('nautalis.command.kb.list', { 
            category: options.category || '',
  ...
  kbCmd
    ...
    .action(async (id: string, options: { json?: boolean }) => {
      ...
      try {
        await withSpan('nautalis.command.kb.get', { id }, async () => {
          const config = await loadConfig();
          const store = await getStore(config);
          await store.init();
  ...
  kbCmd
    ...
    .action(async (id: string, options: { force?: boolean }) => {
      ...
      try {
        await withSpan('nautalis.command.kb.delete', { id }, async () => {
          const config = await loadConfig();
          const store = await getStore(config);
          await store.init();
