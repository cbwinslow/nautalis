...
export function registerTeamCommand(program: Command): void {
  ...
  teamCmd
    ...
    .action(async (name: string, options: { slug?: string }) => {
      ...
      try {
        await withSpan('nautalis.command.team.create', { name }, async () => {
  ...
  teamCmd
    ...
    .action(async () => {
      ...
      try {
        await withSpan('nautalis.command.team.list', {}, async () => {
          const config = await loadConfig();
          const store = await getStore(config);
          await store.init();
  ...
  teamCmd
    ...
    .action(async (teamIdentifier: string) => {
      ...
      try {
        await withSpan('nautalis.command.team.info', { team: teamIdentifier }, async () => {
          const config = await loadConfig();
          const store = await getStore(config);
          await store.init();
  ...
  teamCmd
    ...
    .action(async (email: string, options: { role: string; team?: string }) => {
      ...
      try {
        await withSpan('nautalis.command.team.invite', { email, role: options.role, team: options.team || '' }, async () => {
          const config = await loadConfig();
          const store = await getStore(config);
          await store.init();
  ...
  teamCmd
    ...
    .action(async (userId: string, role: string, options: { team?: string }) => {
      ...
      try {
        await withSpan('nautalis.command.team.role', { userId, role }, async () => {
          const config = await loadConfig();
          const store = await getStore(config);
          await store.init();
  ...
  teamCmd
    ...
    .action(async (userId: string, options: { team?: string }) => {
      ...
      try {
        await withSpan('nautalis.command.team.remove', { userId }, async () => {
          const config = await loadConfig();
          const store = await getStore(config);
          await store.init();
  ...
  teamCmd
    ...
    .action(async (teamIdentifier: string) => {
      ...
      try {
        await withSpan('nautalis.command.team.members', { team: teamIdentifier }, async () => {
          const config = await loadConfig();
          const store = await getStore(config);
          await store.init();
