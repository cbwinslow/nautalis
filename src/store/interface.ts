...
export interface Store {
  ...
  // Projects
  createProject(project: { teamId: string; name: string; slug?: string; repoPath?: string; repoUrl?: string }): Promise<string>;
  getProject(id: string): Promise<any>;
  getProjectsForTeam(teamId: string): Promise<any[]>;

  // Agents
  upsertAgent(agent: { teamId: string; userId?: string; projectId?: string; toolName: string; toolVersion?: string; instanceId: string; agentName?: string }): Promise<string>;
  getAgentsForTeam(teamId: string): Promise<any[]>;

  // Sessions
  createSession(session: { id: string; agentId: string; teamId: string; projectId?: string; userId?: string; branch?: string }): Promise<void>;
  ...
  listMemories(teamId: string, filters?: { projectId?: string; memoryType?: string; limit?: number }): Promise<Memory[]>;
  findSimilarMemories(embedding: number[], teamId: string, limit?: number, minScore?: number): Promise<MemoryQueryResult[]>;

  // Vector operations
  insertEmbedding(memoryId: string, embedding: number[]): Promise<void>;

  // Knowledge Base
  createKnowledgeBase(entry: KnowledgeBaseEntry): Promise<string>;
  getKnowledgeBase(id: string): Promise<KnowledgeBaseEntry | null>;
  queryKnowledgeBase(query: KnowledgeBaseQuery): Promise<KnowledgeBaseEntry[]>;
  updateKnowledgeBase(id: string, updates: Partial<KnowledgeBaseEntry>): Promise<void>;
  deleteKnowledgeBase(id: string): Promise<void>;
  searchKnowledgeBase(teamId: string, query: string, embedding?: number[], options?: { category?: string; limit?: number }): Promise<KnowledgeBaseEntry[]>;
